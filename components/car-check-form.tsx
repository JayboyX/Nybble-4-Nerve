  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          <p style={{ fontSize: 13, fontWeight: 700, color: "#111827", margin: "0 0 2px" }}>Check Your Vehicle</p>
          <p style={{ fontSize: 12, color: "#9ca3af", margin: 0 }}>Select your car to see its risk profile</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            style={{
              ...
              padding: 4, color: "#9ca3af", borderRadius: 4,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div>
          ...
          <select value={model} onChange={(e) => handleModelChange(e.target.value)} disabled={!make}
            style={{ ...selectStyle, opacity: make ? 1 : 0.5, cursor: make ? "pointer" : "not-allowed", background: make ? "#fff" : "#f9fafb" }}
            onFocus={(e) => { if (make) e.target.style.borderColor = "#DC2626"; }}
            onBlur={(e) => (e.target.style.borderColor = "#e5e7eb")}>
        ...
        <div>
          ...
          <select value={year} onChange={(e) => setYear(e.target.value)} disabled={!model}
            style={{ ...selectStyle, opacity: model ? 1 : 0.5, cursor: model ? "pointer" : "not-allowed", background: model ? "#fff" : "#f9fafb" }}
            onFocus={(e) => { if (model) e.target.style.borderColor = "#DC2626"; }}
            onBlur={(e) => (e.target.style.borderColor = "#e5e7eb")}>
        ...
        <button
          ...
          style={{
            ...
            background: isComplete ? "#DC2626" : "#f3f4f6",
            color: isComplete ? "#fff" : "#9ca3af",
            fontSize: 13, fontWeight: 600,
            cursor: isComplete ? "pointer" : "not-allowed",
            transition: "background 0.15s, color 0.15s",
          }}
          onMouseEnter={(e) => { if (isComplete) (e.currentTarget as HTMLButtonElement).style.background = "#b91c1c"; }}
          onMouseLeave={(e) => { if (isComplete) (e.currentTarget as HTMLButtonElement).style.background = "#DC2626"; }}
        >
